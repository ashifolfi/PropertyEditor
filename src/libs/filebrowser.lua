...
function gui.FileBrowser(filename_p, args, funcOK)
    ...
    local function filechooser()
        ...
        if (ig.BeginPopupModal(args.key, nil, ig.ImGuiWindowFlags_AlwaysAutoResize)) then
            ...
            ig.BeginChild_Str("files", ig.ImVec2_Float(0,desiredY), true, 0)
            
            for i,v in ipairs(curr_dir_dirs) do
                if(ig.Selectable_Bool(v.name.." ->",false,ig.ImGuiSelectableFlags_AllowDoubleClick,ig.ImVec2_Float(0,0))) then 
            ...
            if ig.InputText("pattern",pattern_ed,32,ig.ImGuiInputTextFlags_EnterReturnsTrue) then curr_dir_done = false end
            local doit = false
